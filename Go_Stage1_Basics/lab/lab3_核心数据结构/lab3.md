...
**实验目标**：彻底搞懂 Go 语言中“数组是值类型”这一核心特性，以及如何利用指针打破这一限制。 **C++ 差异点**：在 C++ 中数组名退化为指针，传参默认是引用效果；在 Go 中，传数组等于完全拷贝。
...
🚀 **开始行动** 请在一个 `lab3.1.go` 文件中完成所有代码。为了整洁，你可以把前三个实验封装成 `func task1()`, `func task2()`, `func task3()`，并在 `main` 中逐个调用，最后运行 `project()`。